...
const messageController={
    ...
    getConversations:async(req,res)=>{
        try {
                ...
                const conversations=await features.query.sort('-updatedAt').populate('recipients','avatar fullname')
    ...
    getMessages:async(req,res)=>{
        try {
            const features=new APIfeatures(Message.find({
                $or:[{sender:req.user._id,recipient:req.params.id},
                    {sender:req.params.id,recipient:req.user._id}]
            
            }),req.query)

            const messages=await features.query.sort('-createAt')
